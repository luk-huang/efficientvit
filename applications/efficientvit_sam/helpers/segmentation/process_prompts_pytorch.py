...
from applications.efficientvit_sam.helpers.auto_mask_generator import DemoEfficientViTSamAutomaticMaskGenerator
from applications.efficientvit_sam.helpers.predictors.effvit_sam_pytorch import PyTorchEfficientViTSamPredictor
...
# Change model name to - instead of _
get_weight_url = lambda model_name: f"assets/checkpoints/efficientvit_sam/{model_name}.pt"
...
def get_predictor(model_name):
    ...
    if torch.cuda.is_available():
        efficientvit_sam = create_efficientvit_sam_model(model_name, True, weight_url).cuda().eval()
    else:
        efficientvit_sam = create_efficientvit_sam_model(model_name, True, weight_url).eval()
